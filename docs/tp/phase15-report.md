...
Status: accepted for the pinned Qwen3.8-27B UD-Q4_K_M checkpoint, TP=2 target,
scheduler-owned n-gram speculation, and F16 and FP8 (`fp8_e4m3`) KV. The
FP8+spec target gate is accepted by the direct two-rank oracle and the live
CUDA-graph/unified HTTP regression recorded in
`docs/tp/upstream-readiness-review.md` §17.
...
- The earlier `qwen35_two_slot_oracle` run passed the two-slot page/cancellation/
  release/reuse/reset lifecycle oracle with `PADDOCK_NO_SPEC=1`; the current
  readiness run re-covered cancellation/reuse through production HTTP and the
  direct pipe/overlap oracles.
...
The direct correctness and serving gates are covered above. The rebuilt CUDA pack
was used for the final target runs. The FP8+spec follow-up is now accepted:
`qwen35_tp_spec` passed the FP8 direct oracle, `qwen35_tp_sampled` passed FP8
page/reuse sampling, and the TP=2 FP8+spec graph/unified HTTP smoke passed
concurrency, cancellation and clean shutdown. F16 exact-output identity is not
required for the lossy FP8 lane.
