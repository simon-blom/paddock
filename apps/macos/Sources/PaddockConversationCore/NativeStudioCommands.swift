    ...
    -> O
  {
    ...
    do {
      ...
      case "models":
        ...
        guard ids.allSatisfy(canChat) || ids.allSatisfy(canAudio) else {
          throw ConversationFailure.invalid("Compare models must share text or audio input")
        }
        let before = document
        try change {
      ...
      case "settings", "samplerDefaults":
        ...
        try change { fields in
          ...
          for (key, value) in p {
            ...
            case "webSearchEnabled", "forensicsEnabled", "fileMetadataEnabled":
      ...
      case "preview", "openDocument":
        ...
        if kind == "preview" {
          ...
          part =
            document?.activeMessages.first { $0["id"]?.string == mid }?["content"]?.array?.first {
              $0["attachmentId"]?.string == aid
            }?.object
        }
        ...
        graphVisible = false
        graphArtifact = nil
        documentPreview = document?.fields
