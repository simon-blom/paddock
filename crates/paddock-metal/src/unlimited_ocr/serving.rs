...
impl Generator for UnlimitedOcr {
    ...
    fn vision_budget(&self) -> Option<paddock_engine::generator::VisionBudget> {
        self.vision.as_ref().map(|_| vision::BUDGET)
    }
    ...
    fn forward_multimodal(
        ...
        self.prefill_images(0, chunks.to_vec()).map(Some)
    }
    fn forward_prefill_multimodal(
        ...
        self.prefill_images(slot, chunks.to_vec())
    }
    fn reset(&mut self) {
        self.pending.clear();
        self.encoding.clear();
    ...
    fn forward_batch(
        ...
        if tokens.len() != positions.len() || tokens.len() > self.slots.len() {
            return Err(GenError::Backend(
                "UnlimitedOcr batch shape mismatch".into(),
            ));
        }
        let rows = tokens
            .iter()
    ...
    fn forward_mixed(
        ...
        {
            return Err(GenError::Backend(
                "invalid UnlimitedOcr mixed decode slots".into(),
            ));
        }
        let mut seen = std::collections::HashSet::new();
        if decodes.iter().any(|r| !seen.insert(r.0)) {
            return Err(GenError::Backend("duplicate decode slot".into()));
        }
