...
impl UnlimitedOcr {
    ...
    fn execute_inner(&mut self, rows: &[(usize, u32, u32)], outputs: &[usize]) -> Result<Vec<f32>> {
        ...
        for &(slot, _, pos) in rows {
            let pos = ring(self.slots[slot].prompt, pos as usize).0 as u32;
                ...
                .is_err()
            {
                if self.radix.evict_lru(&mut self.pool).is_none() {
                    return Err(MetalError::Memory("Unlimited-OCR KV pool exhausted".into()));
                }
            }
        }
        ...
        let write_meta = rows
            ...
            .flat_map(|&(slot, _, pos)| {
                [
                    slot as u32,
                    ring(self.slots[slot].prompt, pos as usize).0 as u32,
                ]
            })
            .collect::<Vec<_>>();
        let attention_meta = rows
            .iter()
            .flat_map(|&(slot, _, pos)| {
                [
                    slot as u32,
                    ring(self.slots[slot].prompt, pos as usize).1 as u32,
                ]
            })
            .collect::<Vec<_>>();
        let s = &self.scratch;
        ...
        unsafe {
            ...
            s.meta.write_u32(&attention_meta);
            s.write_meta.write_u32(&write_meta);
            s.rope.write_u32(&rope);
        ...
        for (slot, seq) in self.slots.iter().enumerate() {
            if let Some(layout) = &seq.mm {
                for image in &layout.images {
                    if rows.iter().any(|r| {
                        r.0 == slot
                            && (r.2 as usize) >= image.offset
                            && (r.2 as usize) < image.offset + image.tokens
                    }) {
                        cmd.dispatch(
                            "vis_inject",
                            &[&image.embd, &s.meta, &s.x],
                            &[
                                WIDTH as u32,
                                m as u32,
                                slot as u32,
        ...
        for l in &self.layers {
            ...
            for (x, heads) in [(&s.q, 10), (&s.k, 10)] {
                cmd.dispatch(
                    "uocr_rope",
                    &[x, &s.rope],
                    &[heads, m as u32],
                    [(m * heads as usize * 64).div_ceil(256), 1, 1],
                    256,
                );
            }
            cmd.dispatch(
                "uocr_store",
                &[&s.k, &s.v, &l.keys, &l.values, &s.write_meta, &s.pages],
                &[m as u32, self.page_stride as u32],
                [(m * KVWIDTH).div_ceil(256), 1, 1],
                256,
            );
            let ap = [10, 10, self.page_stride as u32, 0, 0, SPLITS as u32];
            if !tiles.is_empty() {
                cmd.dispatch(
                    "laguna_prefill",
                    &[
            ...
            if !decodes.is_empty() {
                cmd.dispatch(
                    ...
                    &ap,
                    [10, decodes.len(), SPLITS],
                    128,
                );
                cmd.dispatch(
                    "gemma_merge",
                    &[&s.parts, &s.attn, &s.decode_rows],
                    &[10, SPLITS as u32, 128],
                    [decodes.len() * 10, 1, 1],
                    32,
                );
            }
            ...
            if let Some(experts) = &l.experts {
                self.moe.run(&cmd, experts, &s.norm, &s.delta, m);
            }
            residual();
        }
        if !outputs.is_empty() {
...
// Prefix and generated-window addresses are separate from rotary positions.
// A zero prompt is serial warmup: ordinary causal append until real prefill.
fn ring(prompt: usize, pos: usize) -> (usize, usize) {
    if prompt > 0 && pos >= prompt + 128 {
        (prompt + (pos - prompt) % 128, prompt + 127)
    } else {
        (pos, pos)
    }
}
