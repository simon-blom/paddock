...
impl PaddleOcr {
    ...
    fn execute_inner(&mut self, rows: &[(usize, u32, u32)], outputs: &[usize]) -> Result<Vec<f32>> {
        ...
        for &(slot, _, pos) in rows {
                ...
                .is_err()
            {
                if self.radix.evict_lru(&mut self.pool).is_none() {
                    return Err(MetalError::Memory("PaddleOCR KV pool exhausted".into()));
                }
            }
        }
        ...
        let rope = rows
            ...
            .flat_map(|&(slot, _, pos)| {
                self.slots[slot]
                    .mm
                    .as_ref()
                    .map_or([pos, pos, pos, 0], |l| l.position(pos as usize))
            })
            .collect::<Vec<_>>();
        let s = &self.scratch;
        ...
        cmd.dispatch(
            ...
            &[WIDTH as u32, m as u32, 30, 1f32.to_bits()],
        ...
        let norm = |w: &Weight| {
            cmd.dispatch(
                "rms",
                &[&s.x, &w.buffer, &s.norm],
                &[WIDTH as u32, w.ty, 1e-5f32.to_bits()],
                [m, 1, 1],
                256,
            )
        };
        ...
        for l in &self.layers {
            ...
            let ap = [16, 2, self.page_stride as u32, 0, 0, SPLITS as u32];
            if !tiles.is_empty() {
                cmd.dispatch(
                    "laguna_prefill",
                    &[
