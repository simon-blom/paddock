...
impl Qwen3Asr {
    pub fn load(
        ...
        if map.gguf().architecture() != Some("qwen3vl") {
            return Err(error("requires architecture=qwen3vl"));
        }
        for (key, value) in [
        ...
        if !matches!(map.gguf().arch_field("rope.dimension_sections"),Some(Value::Array(a)) if a.iter().map(Value::as_i64).collect::<Option<Vec<_>>>()==Some(vec![24,20,20,0]))
            || map.gguf().arch_field("rope.scaling.type").is_some()
        {
            return Err(error("unsupported rotary sections/scaling"));
        }
        if context == 0 || context > 8192 || max_batch == 0 || max_batch > 16 {
            return Err(error(
                "context must be 1..8192, batch 1..16 (implementation ceilings)",
            ));
        }
        let mut schema = vec![
        ...
        for i in 0..LAYERS {
            for (name, dims, ty) in [
                ...
                ("attn_q_norm.weight", vec![128], 0),
                ("attn_k_norm.weight", vec![128], 0),
        ...
        if !matches!(map.gguf().metadata.get("tokenizer.ggml.tokens"),Some(Value::Array(tokens)) if tokens.get(AUDIO as usize).and_then(Value::as_str)==Some("<|audio_pad|>"))
        {
            return Err(error(
                "audio placeholder token does not match elected checkpoint",
            ));
        }
        ...
        if weight_bytes + kv_bytes + scratch_bytes as u64 > device.budget_bytes() {
            return Err(MetalError::Memory(
                "Qwen3-ASR decoder weights + KV + scratch exceed grant".into(),
            ));
        }
