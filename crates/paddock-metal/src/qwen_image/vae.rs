...
pub(super) struct VaeDecoder {
    exec: Rc<Ops>,
    post_quant_w: Tensor<f32>,
    post_quant_b: Tensor<f32>,
    lat_mean: Tensor<f32>,
    lat_std: Tensor<f32>,
    conv_in: Conv3,
    mid_res: [ResBlock; 2],
...
impl VaeDecoder {
    pub(super) fn load(exec: Rc<Ops>, path: &Path) -> Result<Self, MetalError> {
        let st = SafetensorsFile::open(path).map_err(|e| MetalError::Model(format!("vae: {e}")))?;
        let mut ld = Loader {
            exec: exec.clone(),
            st: &st,
            bytes: 0,
        ...
        if pq.len() != 64 * 64 || post_quant_b.len() != 64 * 4 {
            return Err(MetalError::Model(
                "Qwen-Image VAE post-quant shape mismatch".into(),
            ));
        }
        ld.bytes += pq.len() as u64 * 4;
        let conv_in = ld.conv3("decoder.conv_in")?;
        if (conv_in.c_in, conv_in.c_out) != (64, 1152) {
            return Err(MetalError::Model(
                "expected Qwen-Image-2.1 VAE channels".into(),
            ));
        }
        ...
        for i in 0..5 {
            ...
            if upsampler
                .as_ref()
                .is_some_and(|s| (s.c_in, s.c_out) != (c_out, c_out))
            {
                return Err(MetalError::Model(format!(
                    "VAE upsample {i} channel mismatch"
                )));
            }
            ups.push(UpBlock {
                resnets,
        ...
        if norm_out.len() != 144 * 4 || (conv_out.c_in, conv_out.c_out) != (144, 4) {
            return Err(MetalError::Model(
                "expected Qwen-Image-2.1 RGBA output".into(),
            ));
        }
    ...
    pub(super) fn decode_with(
        ...
        let Some(band) = band_rows else {
            ...
            return exec.to_host_u8_len(&u8s, n);
        };
        ...
        exec.to_host_u8_len(&rgba, out_h * out_w * c_rgba)
    }
    ...
    fn banded_up_block(
        ...
        w: usize,
        blk: &UpBlock,
    ) -> Result<(Tensor<f32>, usize, usize), MetalError> {
        const HALO: usize = 7;
        let exec = &self.exec;
    ...
    fn head(&self, x: &Tensor<f32>, h: usize, w: usize) -> Result<Tensor<f32>, MetalError> {
        let exec = &self.exec;
