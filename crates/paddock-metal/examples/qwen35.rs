...
//! cargo run --release -p paddock-metal --example qwen35 -- MODEL PROMPT [N]
#[cfg(target_os = "macos")]
...
fn main() {
    eprintln!("the Qwen35 Metal example requires an M5 Mac");
    std::process::exit(1);
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let path = Path::new(args.get(1).ok_or("usage: qwen35 MODEL PROMPT [N]")?);
    ...
    if prompt == "--inventory" {
        for (k, v) in &mapped.gguf().metadata {
            if k.starts_with("qwen35.") || k.starts_with("dflash.") {
                println!("{k}: {v:?}");
            }
        }
        for t in &mapped.gguf().tensors {
            if t.name.starts_with("blk.0.")
                || t.name.starts_with("blk.64.")
                || t.name.starts_with("blk.3.")
                || !t.name.starts_with("blk.")
            {
                println!("{} {:?} {:?}", t.name, t.dims, t.ggml_type);
            }
        }
        return Ok(());
    }
    ...
    let mut model = paddock_metal::Qwen35::load(path, (tokens.len() + n + 1).max(512), 2, None)?;
